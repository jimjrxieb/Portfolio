...
export default function ChatBox() {
  ...
  async function onSend(e) {
    ...
    try {
      const res = await chat(
        { message: q, namespace: 'portfolio', k: 5 },
        controller.signal
      );
      setHistory(h => [
        ...h,
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3" data-dev="chat-box">
      <div className="flex-1 overflow-auto rounded-xl border p-3">
        {history.length === 0 && (
          <div className="opacity-70">
            Try asking: &ldquo;Tell me about the Jade project&rdquo;
          </div>
        )}
        {history.map((m, i) => (
          <div
          ...
          >
            <div
              className={`inline-block rounded-2xl px-3 py-2 ${m.role === 'user' ? 'bg-blue-500/10' : 'bg-neutral-500/10'}`}
            >
      ...
      {error && (
        <div className="text-red-500 text-sm" data-dev="chat-error">
          {error}
        </div>
      )}
