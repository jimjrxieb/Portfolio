    ...
    <div className="space-y-4" data-dev="projects">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {projects.sections.map((p, i) => (
          <div key={i} className="rounded-2xl border border-jade-200 p-4 shadow-sm">
            ...
            {p.links?.length ? (
              <div className="mt-3 flex gap-3 flex-wrap">
                {p.links.map((l, idx) => (
                  <a key={idx} className="text-sm underline" href={l.href} target="_blank" rel="noreferrer">{l.label}</a>
                ))}
              </div>
            ) : null}
          </div>
