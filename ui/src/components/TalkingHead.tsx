...
export const TalkingHead = React.forwardRef<TalkingHeadRef, TalkingHeadProps>(
  ({ onReady, onSpeaking, className = '', speaking = false, onAnimationComplete }, ref) => {
    const [currentMouth, setCurrentMouth] = useState<keyof typeof MOUTH_FRAMES>('closed');
    ...
    const startBlinking = useCallback(() => {
      const blink = () => {
        if (!isSpeaking || Math.random() > 0.7) { // Reduce blinks while speaking
          setIsBlinking(true);
    ...
    const startEyeDarts = useCallback(() => {
      const dart = () => {
        if (!isSpeaking) { // Only dart when not speaking
    ...
    const processVisemes = useCallback((visemes: VisemeData[]) => {
      let currentIndex = 0;
      ...
      const animate = () => {
        ...
        if (currentIndex < visemes.length) {
          ...
          switch (viseme.phoneme?.toUpperCase()) {
            case 'A': case 'AA': case 'AH': mouthShape = 'A'; break;
            case 'E': case 'EH': case 'ER': mouthShape = 'E'; break;
            case 'I': case 'IH': case 'IY': mouthShape = 'I'; break;
            case 'O': case 'OH': case 'AO': mouthShape = 'O'; break;
            case 'U': case 'UH': case 'UW': mouthShape = 'U'; break;
            default: mouthShape = 'closed';
          }
    ...
    const speak = useCallback(async (ttsData: TTSData) => {
      ...
      if (ttsData.visemes && ttsData.visemes.length > 0) {
        ...
        const simpleTalk = () => {
          elapsed += 150;
          const frames: (keyof typeof MOUTH_FRAMES)[] = ['A', 'E', 'I', 'O', 'U', 'closed'];
