...
test("Chat answers and shows model banner", async ({ page }) => {
  ...
  await expect(page.getByText(/ZRS|Jade|RAG|property|management/i)).toBeVisible({ timeout: 20000 });
});
...
test("Content sources work correctly", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /Live Demo|Create Your Avatar/i })).toBeVisible();
...
test("Avatar panel functionality", async ({ page }) => {
  ...
  const playButton = avatarPanel.getByRole("button", { name: /Play Introduction/i });
