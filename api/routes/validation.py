...
class ValidationRequest(BaseModel):
    ...
    context_sources: List[str] = Field(default=[], description="Sources used for context")
...
GROUNDING_KEYWORDS = {
    "jimmie_identity": ["Jimmie Coleman", "DevSecOps", "CompTIA Security+", "CKA", "LinkOps"],
    "linkops_project": ["LinkOps AI-BOX", "Jade Box", "plug-and-play", "security-conscious", "local deployment"],
    "zrs_client": ["ZRS Management", "Orlando", "property management", "compliance", "productivity"],
    "technical_stack": ["FastAPI", "React", "Docker", "Kubernetes", "ChromaDB", "GitHub Actions"],
    ...
    for rule in CONTEXT_VALIDATION_RULES:
        ...
        if any(keyword in question_lower or keyword in text_lower 
            ...
            if rule["required_context"]:
                has_required = any(ctx.lower() in text_lower 
            ...
            if rule["forbidden_claims"]:
                for forbidden in rule["forbidden_claims"]:
                    if forbidden == "*":
                        ...
                        issues.append(
                            f"Contains forbidden claim: '{forbidden}'"
                        )
...
async def validate_response(request: ValidationRequest) -> ValidationResult:
    ...
    try:
        ...
        for issue in hallucination_issues:
            all_issues.append(f"Hallucination: {issue['description']} - '{issue['matched_text']}'")
        ...
        critical_issues = [i for i in hallucination_issues if i['severity'] == 'critical']
        ...
        confidence_score = max(0, min(1, 
            grounding_score - hallucination_penalty - context_penalty
        ))
        
        # Overall validation
        is_valid = (
            anti_hallucination_passed and 
            grounding_score >= 0.4 and 
...
async def test_response_safety(
    ...
    try:
        ...
        return {
            ...
            "recommendation": (
                "✅ SAFE TO USE" if validation_result.is_valid
                else "⚠️ NEEDS REVIEW" if validation_result.confidence_score > 0.5
                else "❌ HIGH RISK"
            )
        }
