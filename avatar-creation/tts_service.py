...
class VisemeMapping:
    ...
    VISEME_TO_VRM = {
        ...
        9: {
            "name": "ae",
            "blendshapes": {"A": 0.7, "E": 0.3, "jawOpen": 0.4},
        },  # ae (bat)
        10: {
            "name": "ey",
            "blendshapes": {"E": 0.8, "I": 0.2, "jawOpen": 0.2},
        },  # ey (bait)
        11: {
            "name": "ay",
            "blendshapes": {"A": 0.7, "I": 0.3, "jawOpen": 0.3},
        },  # ay (bite)
        12: {
            "name": "oy",
            "blendshapes": {"O": 0.7, "I": 0.3, "jawOpen": 0.3},
        },  # oy (boy)
        13: {
            "name": "aw",
            "blendshapes": {"A": 0.6, "U": 0.4, "jawOpen": 0.4},
        },  # aw (bout)
        14: {
            "name": "ow",
            "blendshapes": {"O": 0.8, "U": 0.2, "jawOpen": 0.3},
        },  # ow (boat)
...
class TTSService:
    ...
    def _on_viseme_received(self, evt):
        ...
        viseme_info = {
            ...
            "blendshapes": VisemeMapping.VISEME_TO_VRM.get(evt.viseme_id, {}).get(
                "blendshapes", {}
            ),
            "phoneme": VisemeMapping.VISEME_TO_VRM.get(evt.viseme_id, {}).get(
                "name", "unknown"
            ),
        }
    ...
    async def synthesize_with_visemes(self, text: str) -> Dict:
        ...
        result = await loop.run_in_executor(None, lambda: synthesizer.speak_ssml(ssml))
