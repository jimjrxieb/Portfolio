...
class ConversationEngine:
    ...
    def analyze_question_intent(self, question: str) -> Dict[str, str]:
        ...
        category = "general"
        if any(word in question_lower for word in ["technical", "technology", "how does", "architecture", "built"]):
            category = "technical"
        elif any(word in question_lower for word in ["business", "roi", "cost", "savings", "impact"]):
            category = "business"
        elif any(word in question_lower for word in ["project", "ai-box", "afterlife", "jade"]):
            category = "projects"
        elif any(word in question_lower for word in ["yourself", "background", "experience", "who"]):
            category = "introduction"
        
        # Determine desired depth
        depth = "overview"
        if any(word in question_lower for word in ["detail", "deep", "explain", "how exactly"]):
            depth = "detailed"
        elif any(word in question_lower for word in ["implementation", "code", "architecture", "specific"]):
            depth = "deep"
    ...
    def generate_response(self, 
                         ...
                         rag_results: Optional[List[str]] = None) -> str:
        """
    ...
    def _generate_contextual_response(self, 
        ...
        if intent["category"] == "projects":
            return self._generate_project_response(intent["mentioned_projects"], intent["depth"])
        ...
        for project_key in projects:
            if project_key in self.project_descriptions:
                ...
                if depth == "overview":
                    ...
                    responses.append(f"{project['name']} - {project['summary']}. Key features include: {features}")
                else:  # deep
                    features = ". ".join(project['key_features'])
                    tech = ", ".join(project['tech_stack'])
                    responses.append(f"{project['name']} - {project['summary']}. Technical implementation uses {tech}. {features}. {project['business_impact']}.")
        
        return " ".join(responses)
    
    def _generate_technical_response(self, question: str, rag_results: Optional[List[str]]) -> str:
    ...
    def _generate_business_response(self, question: str, rag_results: Optional[List[str]]) -> str:
