...
def test_health_endpoint_truthfulness():
    ...
    try:
        ...
        print(f"✅ Health endpoint returns: {health_data['llm_provider']}/{health_data['llm_model']}")
...
def test_chat_response_grounding():
    ...
    try:
        ...
        print(f"✅ Response properly grounded with {len(chat_data['citations'])} citations")
...
def test_negative_case_handling():
    ...
    try:
        ...
        if not refused:
            print(f"❌ Should refuse out-of-scope question but answered: {answer[:100]}...")
            return False
