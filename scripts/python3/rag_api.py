...
app = FastAPI(
    ...
    description="Grounded RAG system for Jimmie's portfolio",
    version="1.0.0",
    docs_url="/docs" if os.getenv("DEBUG_MODE") else None,
    redoc_url="/redoc" if os.getenv("DEBUG_MODE") else None,
)

# Security middleware stack
app.add_middleware(
    TrustedHostMiddleware,
    allowed_hosts=["linksmlm.com", "localhost", "127.0.0.1", "*.linksmlm.com"],
)
...
PROMPT_INJECTION_PATTERNS = [
    r"ignore.{0,20}(previous|above|system|instructions?)",
    r"(system|assistant|user)\s*[:=]\s*[\"']",
    r"\[\s*(system|assistant|user)\s*\]",
    r"<\s*(system|assistant|user)\s*>",
    r"(override|bypass|disable).{0,20}(security|safety|filter)",
    r"(jailbreak|DAN|developer.mode)",
    r"act.as.if.you.are",
    r"pretend.{0,10}(you.are|to.be)",
    r"(new|different).{0,10}(persona|character|role)",
]
...
class ChatRequest(BaseModel):
    ...
    def validate_message(cls, v):
        ...
        if special_char_count > 20:
            raise ValueError("Excessive special characters")
    ...
    def validate_namespace(cls, v):
        if not isinstance(v, str) or not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "Invalid namespace format - only alphanumeric, underscore, hyphen allowed"
            )
        if len(v) > 50:
...
def check_rate_limit(client_id: str) -> tuple[bool, int]:
    ...
    # Clean old windows
    old_windows = [
        w for w in client_data.keys() if w < window_start - RATE_LIMIT_WINDOW
    ]
    for w in old_windows:
        del client_data[w]

    # Count requests in current window
    current_requests = client_data.get(window_start, 0)

    if current_requests >= RATE_LIMIT_MAX_REQUESTS:
        return False, 0

    # Increment counter
    client_data[window_start] = current_requests + 1
    remaining = RATE_LIMIT_MAX_REQUESTS - client_data[window_start]
    ...
    for chunk_id, chunk in MOCK_KNOWLEDGE_BASE.items():
        ...
        if any(
            ...
            if any(
                ...
                score += 0.6
        if any(
            term in query_lower for term in ["linkops", "ai-box", "property", "jade"]
        ):
            if "linkops" in text_lower or "ai-box" in text_lower:
                score += 0.6
        if any(
            term in query_lower
            for term in ["technical", "stack", "technologies", "tech"]
        ):
        ...
        if any(
            ...
            if "introduction" in chunk_id or any(
                intro_term in text_lower
                for intro_term in [
                    "aspiring",
                    "cloud",
                    "platform engineer",
        ...
        if any(
            term in query_lower
            for term in ["aks", "helm", "trivy", "sonarqube", "bicep", "terraform"]
        ):
            if "azure_devops_qa" in chunk_id:
                score += 0.8
        if any(
...
def create_grounded_prompt(query: str, context_chunks: List[Dict]) -> str:
    """Create prompt that forces grounding to provided context with injection protection"""

    # Security: Additional prompt injection check
    ...
    for stopword in injection_stopwords:
        sanitized_query = sanitized_query.replace(stopword, "")
...
# Phase 4 & 5 - Main chat endpoint with security & observability
@app.post("/api/chat", response_model=ChatResponse)
async def chat(request: ChatRequest, http_request: Request, response: Response):
    """Grounded RAG chat endpoint with security controls"""
    ...
    # Debug: Uncomment for rate limit debugging
    # print(f"RATE LIMIT DEBUG: client_id={client_id}, allowed={allowed}, remaining={remaining}")
    ...
    # Security: Add protective headers
    add_security_headers(response)

    try:
        # Phase 2: Retrieve relevant context
        context_chunks = retrieve_context(
            request.message, k=request.k, threshold=CONFIG["retrieval_threshold"]
        )
...
# Phase 0 & 5 - Proper health endpoint with security
...
# Phase 5 - Debug endpoint with security controls
@app.get("/debug/retrieval")
def debug_retrieval(query: str, k: int = 5, response: Response = None):
    """Debug endpoint with security validation"""
    if not os.getenv("DEBUG_MODE"):
        raise HTTPException(status_code=404, detail="Not found")

    add_security_headers(response)

    # Security: Validate debug query
    if len(query) > 200:
    ...
    context_chunks = retrieve_context(
        query, k=k, threshold=CONFIG["retrieval_threshold"]
    )
...
# Phase 7 - Quick prompts with security headers
@app.get("/api/chat/prompts")
def get_quick_prompts(response: Response):
    """Get curated prompts with security headers"""
    add_security_headers(response)
