...
class ChatRequest(BaseModel):
    ...
    def validate_namespace(cls, v):
        if not isinstance(v, str) or not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("Invalid namespace format - only alphanumeric, underscore, hyphen allowed")
        if len(v) > 50:
...
def check_rate_limit(client_id: str) -> tuple[bool, int]:
    ...
    # Clean old windows
    old_windows = [w for w in client_data.keys() if w < window_start - RATE_LIMIT_WINDOW]
...
def create_grounded_prompt(query: str, context_chunks: List[Dict]) -> str:
    ...
    injection_stopwords = ["system:", "assistant:", "user:", "[system]", "[assistant]", "[user]", 
                          "<system>", "<assistant>", "<user>", "ignore previous", "disregard", 
                          "forget", "override", "new instructions"]
...
async def chat(request: ChatRequest, http_request: Request, response: Response):
    ...
    try:
        ...
        if "validation" in str(e).lower() or "invalid" in str(e).lower():
            ...
            raise HTTPException(status_code=500, detail=f"Processing failed (ref: {error_id})")
