...
class DocumentProcessor:
    ...
    def _extract_tags(self, text: str, filename: str) -> List[str]:
        ...
        if any(
            term in text_lower for term in ["phi-3", "gpt", "llm", "language model"]
        ):
            tags.append("llm")
        if any(
            term in text_lower for term in ["chromadb", "vector database", "embeddings"]
        ):
            tags.append("rag")
        ...
        if any(
            term in text_lower for term in ["security", "compliance", "hipaa", "gdpr"]
        ):
            tags.append("security")
        if any(
            term in text_lower
            for term in ["property management", "tenant", "maintenance"]
        ):
...
def main():
    ...
    knowledge_dir = (
        Path(__file__).parent.parent.parent / "data" / "knowledge" / "jimmie"
    )
    ...
    if ingested_count > 0:
        logger.info(
            f"Successfully ingested {ingested_count} documents to {version_name}"
        )
    ...
    for query in test_queries:
        ...
        if results:
            ...
            logger.info(
                f"Top result from: {top_result['metadata'].get('source', 'Unknown')}"
            )
