...
def test_kubernetes_security():
    ...
    security_ok = all(
        security_context.get(key) == value 
...
def test_docker_security():
    ...
    if dockerfile_path.exists():
        ...
        if "USER appuser" in dockerfile_content:
            if "useradd -u 10001" in dockerfile_content and "-s /sbin/nologin" in dockerfile_content:
    ...
    if dockerfile_path.exists():
        ...
        labels_found = sum(1 for label in required_labels if label in dockerfile_content)
...
def run_security_audit():
    ...
    security_percentage = (all_tests_passed / all_total_tests) * 100 if all_total_tests > 0 else 0
