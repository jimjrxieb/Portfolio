    ...
    for py_file in api_dir.rglob('*.py'):
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if (('import routes_' in line) or 
                    ('from routes_' in line) or
                    ('import engines.' in line) or
                    ('from engines.' in line) or
                    ('import services.' in line and 'app.services' not in line) or
                    ('from services.' in line and 'app.services' not in line) or
                    ('import schemas' in line and 'app.schemas' not in line)):
...
def test_imports():
    ...
    try:
        ...
        print(f"✅ API imports successful - {settings.LLM_PROVIDER}/{settings.LLM_MODEL}")
        return True
    except Exception as e:
